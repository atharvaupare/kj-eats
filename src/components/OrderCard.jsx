        ...
        <div className="flex flex-col w-full gap-y-2">
          {order?.cartDetails?.cartItems?.map((item, index) => (
            <>
              <div
                key={index}
                className="flex justify-between w-full text-lg text-darkgray"
              >
                <p>
          ...
          <div className="w-full border border-dark-main"></div>
          <p className="text-2xl font-bold">Total Amount: {order?.cartDetails?.totalAmount}</p>
        </div>
      </div>
    </div>
  );
